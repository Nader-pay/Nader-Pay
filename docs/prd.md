...
```
Nader Pay Agent
├── شاشة التحقق من الإصدار (Version Check) [جديد]
...
└── الإعدادات (Settings)
    ├── حول التطبيق (About) [جديد]
    └── التشخيص (Diagnostics)
```

---

### 4.2 شاشة التحقق من الإصدار (Version Check) [جديد]

**الغرض:** التحقق من توفر إصدار أحدث على GitHub Releases عند كل تشغيل للتطبيق، وإجبار المستخدم على التحديث إذا كان إصدار أحدث متاحاً.

**التوقيت:** تُشغَّل هذه الشاشة/العملية فور بدء تشغيل التطبيق قبل الوصول إلى أي شاشة أخرى.

**1. مصدر بيانات الإصدار:**
- يتحقق التطبيق من أحدث إصدار عبر GitHub Releases API للمستودع المحدد
- يقارن رقم الإصدار الحالي المضمّن في التطبيق برقم الإصدار الأحدث على GitHub Releases
- مقارنة الإصدارات تتبع Semantic Versioning (MAJOR.MINOR.PATCH)

**2. حالات التحقق:**

| الحالة | السلوك |
|---|---|
| الإصدار الحالي هو الأحدث | المتابعة الطبيعية لتشغيل التطبيق |
| يوجد إصدار أحدث | عرض شاشة إجبار التحديث — لا يمكن تجاوزها |
| فشل الاتصال بـ GitHub Releases API | المتابعة الطبيعية مع تسجيل الخطأ في Diagnostics |
| الجهاز Offline | المتابعة الطبيعية — لا يُوقف التطبيق بسبب عدم الاتصال |

**3. شاشة إجبار التحديث:**
- تُعرض عند اكتشاف إصدار أحدث وتحجب الوصول الكامل للتطبيق
- تحتوي على:
  - اسم التطبيق: Nader Pay Agent
  - رقم الإصدار الحالي
  - رقم الإصدار الجديد المتاح
  - نص توضيحي: «يتوفر إصدار جديد من التطبيق. يجب التحديث للمتابعة.»
  - زر «تحديث الآن»: يفتح رابط تنزيل APK من GitHub Releases مباشرة (ملف nader-pay-agent-v{VERSION}.apk)
- لا يوجد زر «تخطي» أو «لاحقاً» — التحديث إجباري
- الشاشة لا تُغلق ولا يمكن تجاوزها بالضغط على زر الرجوع

**4. قاعدة صارمة:** فشل التحقق من الإصدار (Offline أو خطأ شبكة) لا يوقف التطبيق ولا يعرض شاشة إجبار التحديث.

---

### 4.3 شاشة الرئيسية / Dashboard
...
### 4.5 شاشة تفاصيل الطلب (Order Details)
...
### 4.6 شاشة مصادر الدفع (Payment Sources)

#### 4.6.1 قائمة Providers
...
#### 4.6.3 شاشة اكتشاف واختيار مصادر SMS [جديد]
...
- شريط تصفية في أعلى الشاشة يحتوي على: كل المصادر، Vodafone Cash، Orange Cash، InstaPay، Bank Transfer
...
- عند الضغط على «اختيار»: عرض نافذة تأكيد تحتوي على: Sender Address المختار، Provider المرتبط به، عدد الرسائل المتاحة، معاينة آخر رسالة (Raw SMS بدون تعديل)، زر «تأكيد الاختيار» وزر «إلغاء»
...
- نموذج يحتوي على: حقل Sender Address (رقم أو معرّف)، اختيار Provider من قائمة (Vodafone Cash / Orange Cash / InstaPay / Bank Transfer)، زر «إضافة»
...
#### 4.6.4 توثيق المصدر (Source Verification)
...
#### 4.6.7 Live Status لكل Provider
...
### 4.7 شاشة ملفات الخادم (Server Profiles)
...
10. حول التطبيق (About) [جديد]
11. التشخيص (Diagnostics)

#### 4.8.1 قسم حول التطبيق (About) [جديد]

**البيانات المعروضة:**
- اسم التطبيق: Nader Pay Agent
- رقم الإصدار الحالي (App Version) — مشتق من إعدادات البناء الفعلية
- رقم الإصدار الأحدث المتاح على GitHub Releases (إن كان متاحاً من آخر فحص)
- حالة الإصدار: «أحدث إصدار» أو «يتوفر إصدار جديد: v{VERSION}»
- زر «التحقق من التحديثات»: يُشغّل فحص الإصدار يدوياً ويعرض النتيجة فوراً
- إذا كان يتوفر إصدار جديد: يظهر زر «تحديث الآن» يفتح رابط تنزيل APK من GitHub Releases

**قاعدة صارمة:** رقم الإصدار المعروض مشتق من إعدادات البناء الفعلية — لا يُكتب يدوياً في الكود.
...
### 5.6 نظام إدارة الإصدارات والتحديث الإجباري [جديد]

#### 5.6.1 فحص الإصدار عند البداية

- عند كل تشغيل للتطبيق: يُرسَل طلب إلى GitHub Releases API للمستودع المحدد لجلب أحدث إصدار منشور
- يُقارَن رقم الإصدار الحالي المضمّن في التطبيق برقم الإصدار الأحدث باستخدام Semantic Versioning
- إذا كان الإصدار الأحدث أعلى من الإصدار الحالي: تُعرض شاشة إجبار التحديث وتحجب الوصول الكامل
- إذا كان الإصدار الحالي هو الأحدث أو مساوياً: المتابعة الطبيعية
- إذا فشل الطلب (Offline أو خطأ شبكة أو خطأ GitHub API): المتابعة الطبيعية وتسجيل الخطأ في Diagnostics

#### 5.6.2 شاشة إجبار التحديث

- تحجب الوصول الكامل للتطبيق — لا يمكن تجاوزها
- لا يعمل زر الرجوع في Android لإغلاقها
- تعرض: رقم الإصدار الحالي، رقم الإصدار الجديد، زر «تحديث الآن» يفتح رابط تنزيل APK من GitHub Releases (nader-pay-agent-v{VERSION}.apk)
- بعد تثبيت الإصدار الجديد وإعادة تشغيل التطبيق: يُشغَّل مسار Clean Migration تلقائياً

#### 5.6.3 Clean Migration عند التحديث

- عند أول تشغيل بعد تثبيت إصدار جديد: يتحقق التطبيق من رقم الإصدار المحفوظ محلياً مقارنةً بالإصدار الحالي
- إذا اختلف الإصدار: يُشغَّل مسار Migration
- مسار Migration:
  1. مسح الكاش والبيانات المؤقتة غير المتوافقة
  2. تشغيل SQLite migrations المطلوبة لتحديث هيكل قاعدة البيانات
  3. الحفاظ على بيانات الإنتاج: Server Profiles، Provider Sources الموثقة، سجلات الطلبات، بيانات المستخدمين
  4. حذف البيانات غير المتوافقة مع الإصدار الجديد فقط (محددة في migration scripts)
  5. تحديث رقم الإصدار المحفوظ محلياً
  6. تشغيل التطبيق بنظامه الجديد
- إذا فشل Migration: عرض رسالة خطأ واضحة في Diagnostics مع تفاصيل الفشل
- ممنوع حذف: بيانات الإنتاج أو بيانات المستخدمين أو سجلات الخادم

#### 5.6.4 تسمية ملف APK

- اسم ملف APK المُنتَج من GitHub Actions: nader-pay-agent-v{VERSION}.apk
- حيث {VERSION} هو رقم الإصدار الفعلي المضمّن في إعدادات البناء (مثال: nader-pay-agent-v1.2.3.apk)
- يُرفع الملف على GitHub Releases مع كل إصدار جديد
- رقم الإصدار في اسم الملف يطابق رقم الإصدار المضمّن في التطبيق

#### 5.6.5 GitHub Actions — بناء APK ورفعه

- GitHub Actions يبني APK ويوقّعه ويرفعه على GitHub Releases تلقائياً
- اسم الملف المرفوع: nader-pay-agent-v{VERSION}.apk
- رقم الإصدار مشتق من إعدادات البناء (app.json أو package.json) — لا يُكتب يدوياً في workflow
- التوقيع يتم باستخدام Android Keystore المحفوظ في GitHub Secrets

---

### 5.7 نظام Source Verification — القواعد الجوهرية

#### 5.7.1 نموذج بيانات Provider في SQLite (جدول provider_sources)
...
#### 5.7.2 قاعدة SMS Reader — مطابقة المصدر بنوع الطلب
...
### 5.11 SMS Reader وLocal Transaction Ledger
...
### 5.12 Order ↔ SMS Matching (محافظ عليه)
...
### 5.13 Verification Pipeline (محافظ عليه — لا تعديل)
...
### 5.14 Order State Machine (محافظ عليه)
...
### 5.15 Transaction Deduplication (محافظ عليه)
...
### 5.17 Permission Manager الموحد
...
### 5.18 وضع عدم الاتصال (Offline-First) — تعزيز
...
### 5.19 Live Synchronization (محافظ عليه)
...
### 5.20 Background Agent (محافظ عليه)
...
**أحداث Version Check المضافة [جديد]:**
- Version check started
- Version check passed (current version is latest)
- Update required (current version، latest version)
- Version check failed (reason)
- Migration started (from version، to version)
...
### 5.22 Device Registration (محافظ عليه)
...
### 5.23 Order Normalizer (محافظ عليه)
...
- Android Keystore لتوقيع APK — المفتاح محفوظ في GitHub Secrets ولا يُضمَّن في repository
...
| يتوفر إصدار أحدث على GitHub Releases | عرض شاشة إجبار التحديث — لا يمكن تجاوزها |
| فشل فحص الإصدار (Offline أو خطأ شبكة) | المتابعة الطبيعية وتسجيل الخطأ في Diagnostics |
| أول تشغيل بعد تثبيت إصدار جديد | تشغيل مسار Clean Migration تلقائياً |
| فشل Migration | عرض رسالة خطأ في Diagnostics، عدم حذف بيانات الإنتاج |
...
3. تطوير شاشة اكتشاف واختيار مصادر SMS
4. تطوير Source Verification Engine
5. حفظ نتيجة التوثيق في SQLite
...
### المرحلة I — Version Management [جديد]
1. تطوير منطق فحص الإصدار عبر GitHub Releases API
2. تطوير شاشة إجبار التحديث (غير قابلة للتجاوز)
3. تطوير مسار Clean Migration عند أول تشغيل بعد تثبيت إصدار جديد
4. تطوير قسم About في شاشة الإعدادات مع عرض رقم الإصدار وزر التحقق اليدوي
5. إعداد GitHub Actions workflow لبناء APK وتوقيعه ورفعه على GitHub Releases باسم nader-pay-agent-v{VERSION}.apk
6. التأكد من أن رقم الإصدار مشتق من إعدادات البناء (app.json) في جميع المواضع

**التحقق:** اختبار فحص الإصدار مع إصدار أحدث، اختبار فحص الإصدار في وضع Offline، اختبار مسار Migration، التحقق من اسم ملف APK

### المرحلة J — Agent Services
...
4. تحديث Activity Log بأحداث PROVIDER_MISMATCH والأحداث الجديدة

### المرحلة L — Validation
...
### L — شاشة اكتشاف واختيار مصادر SMS
...
### M — Version Management [جديد]
- فحص الإصدار عند التشغيل مع إصدار حالي هو الأحدث: المتابعة الطبيعية
- فحص الإصدار عند التشغيل مع وجود إصدار أحدث: عرض شاشة إجبار التحديث
- محاولة تجاوز شاشة إجبار التحديث بزر الرجوع: الشاشة تبقى
- فحص الإصدار في وضع Offline: المتابعة الطبيعية وتسجيل الخطأ في Diagnostics
- فحص الإصدار مع خطأ GitHub API: المتابعة الطبيعية وتسجيل الخطأ في Diagnostics
- أول تشغيل بعد تثبيت إصدار جديد: تشغيل مسار Migration تلقائياً
- Migration ناجح: بيانات الإنتاج محفوظة، التطبيق يعمل بالإصدار الجديد
- Migration فاشل: رسالة خطأ في Diagnostics، بيانات الإنتاج غير محذوفة
- قسم About: عرض رقم الإصدار الصحيح المشتق من إعدادات البناء
- زر «التحقق من التحديثات» في About: فحص يدوي فوري وعرض النتيجة
- اسم ملف APK المُنتَج: nader-pay-agent-v{VERSION}.apk
...
- Version Management: فحص الإصدار عبر GitHub Releases API، شاشة إجبار التحديث، مسار Clean Migration، تسمية APK، GitHub Actions workflow
- Testing وdeployment
...
34. التطبيق يتحقق من أحدث إصدار على GitHub Releases عند كل تشغيل
35. شاشة إجبار التحديث لا يمكن تجاوزها عند وجود إصدار أحدث
36. فشل فحص الإصدار (Offline أو خطأ شبكة) لا يوقف التطبيق
37. رقم الإصدار معروض بشكل صحيح في قسم About مشتق من إعدادات البناء الفعلية
38. مسار Clean Migration يُشغَّل تلقائياً عند أول تشغيل بعد تثبيت إصدار جديد
39. Migration لا يحذف بيانات الإنتاج أو بيانات المستخدمين أو سجلات الخادم
40. اسم ملف APK المُنتَج من GitHub Actions: nader-pay-agent-v{VERSION}.apk
...
- تحديث تلقائي صامت بدون تدخل المستخدم (OTA silent update)
- إدارة قنوات توزيع متعددة غير GitHub Releases