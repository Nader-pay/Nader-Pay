...
const dbReady = SQLite.openDatabaseAsync(DB_NAME, DB_OPTIONS)
  ...
  .then(async (db) => {
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_provider_source_unique ON provider_sources(provider_id, source_id);
    CREATE INDEX IF NOT EXISTS idx_provider_source_verified ON provider_sources(provider_id, verified);
    ...
    CREATE INDEX IF NOT EXISTS idx_source_logs_provider ON source_verification_logs(provider_id, source_id, created_at DESC);
  `;
...
export async function getProviderSource(
  providerId: string,
  sourceId: string
): Promise<{
  id: number;
  ...
  return (await db.getFirstAsync(
    'SELECT * FROM provider_sources WHERE provider_id = ? AND source_id = ?',
    [providerId, sourceId]
  )) as any;
}
...
export async function getVerifiedProviderSource(
  providerId: string
): Promise<{
  id: number;
  provider_id: string;
  ...
  const db = await dbReady;
  return (await db.getFirstAsync(
    'SELECT * FROM provider_sources WHERE provider_id = ? AND verified = 1 AND enabled = 1 ORDER BY updated_at DESC LIMIT 1',
    [providerId]
  )) as any;
}

export async function saveProviderSource(input: {
  providerId: string;
  sourceId: string;
  sourceType: string;
  ...
  await db.runAsync(
    `INSERT INTO provider_sources (
      provider_id, source_id, source_type, source_metadata,
      verified, enabled, last_verification_at, last_verification_result,
      last_message_at, last_message_summary, parser_version
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    [
      ...
      input.lastVerificationAt ?? null,
      input.lastVerificationResult ?? null,
      input.lastMessageAt ?? null,
      input.lastMessageSummary ?? null,
      input.parserVersion ?? null,
    ]
  );
}

export async function updateProviderSourceVerification(
  providerId: string,
  sourceId: string,
  verified: boolean,
  result?: string | null
  ...
  await db.runAsync(
    ...
    [verified ? 1 : 0, result ?? null, providerId, sourceId]
  );
}

export async function revokeProviderSource(providerId: string): Promise<void> {
  const db = await dbReady;
  await db.runAsync(
    `UPDATE provider_sources SET
      verified = 0,
      enabled = 0,
      last_verification_result = 'revoked',
      updated_at = datetime('now')
     WHERE provider_id = ?`,
    [providerId]
  );
...
export async function deleteProviderSource(providerId: string, sourceId: string): Promise<void> {
  const db = await dbReady;
  await db.runAsync('DELETE FROM provider_sources WHERE provider_id = ? AND source_id = ?', [
    providerId,
    sourceId,
  ]);
}

export async function setProviderSourceEnabled(
  providerId: string,
  sourceId: string,
  enabled: boolean
): Promise<void> {
  const db = await dbReady;
  await db.runAsync(
    `UPDATE provider_sources SET enabled = ?, updated_at = datetime('now') WHERE provider_id = ? AND source_id = ?`,
    [enabled ? 1 : 0, providerId, sourceId]
  );
}

export async function updateProviderSourceLastMessage(
  providerId: string,
  sourceId: string,
  ...
  await db.runAsync(
    `UPDATE provider_sources SET
      ...
      updated_at = datetime('now')
     WHERE provider_id = ? AND source_id = ?`,
    [lastMessageAt, lastMessageSummary, providerId, sourceId]
  );
}

export async function logSourceVerification(
  providerId: string,
  sourceId: string,
  action: string,
  result?: string | null,
  ...
  const db = await dbReady;
  await db.runAsync(
    'INSERT INTO source_verification_logs (provider_id, source_id, action, result, reason, payload) VALUES (?, ?, ?, ?, ?, ?)',
    [providerId, sourceId, action, result ?? null, reason ?? null, payload ? JSON.stringify(payload) : null]
  );
}

export async function getSourceVerificationLogs(providerId: string, limit = 50): Promise<
  {
    id: number;
    provider_id: string;
    source_id: string;
...
> {
  const db = await dbReady;
  return db.getAllAsync(
    'SELECT * FROM source_verification_logs WHERE provider_id = ? ORDER BY created_at DESC LIMIT ?',
    [providerId, limit]
  ) as any;
}
