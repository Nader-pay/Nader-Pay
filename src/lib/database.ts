...
export const dbReady = SQLite.openDatabaseAsync(DB_NAME, DB_OPTIONS)
  ...
  // ── Phase 4 Migrations (Phase 1 — Trusted Sources) ──────────────────────

  // notification_sources: مصادر إشعارات موثوقة لكل provider (InstaPay أولاً)
  const hasNotifSources = (await db.getAllAsync<{ name: string }>(
    "SELECT name FROM sqlite_master WHERE type='table' AND name='notification_sources'"
  )).length > 0;
  if (!hasNotifSources) {
    await db.execAsync(`
    ...
    `);
    await db.execAsync("CREATE INDEX IF NOT EXISTS idx_notif_sources_provider ON notification_sources(provider_id)");
    await db.execAsync("CREATE INDEX IF NOT EXISTS idx_notif_sources_status ON notification_sources(status)");
  }

  // provider_sources: إضافة parser_id + parser_version + verification_stage
  const psCols4 = await db.getAllAsync<{ name: string }>("PRAGMA table_info('provider_sources')");
  const psColNames4 = new Set(psCols4.map((c) => c.name));
  if (!psColNames4.has('parser_id')) {
    await db.execAsync("ALTER TABLE provider_sources ADD COLUMN parser_id TEXT");
  }
  if (!psColNames4.has('verification_stage')) {
    await db.execAsync("ALTER TABLE provider_sources ADD COLUMN verification_stage TEXT DEFAULT 'UNVERIFIED'");
  }
  if (!psColNames4.has('verified_at')) {
    await db.execAsync("ALTER TABLE provider_sources ADD COLUMN verified_at TEXT");
  }
