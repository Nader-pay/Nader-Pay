...
export function AgentProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleMessage = useCallback(
    async (message: SmsMessage, currentSettings = settings, currentDevice = deviceState) => {
      ...
      // التحقق من مصدر الرسالة قبل المعالجة: المصدر يجب أن يكون موثقًا للمزود
      if (currentSettings.requireSourceVerification) {
        const sourceCheck = await verifyMessageSource(message);
        if (!sourceCheck.ok) {
          await logVerification(
            ...
            'rejected',
            sourceCheck.reason,
            { provider: sourceCheck.provider, source: message.originatingAddress },
            message.id
          );
          await logEvent('source_untrusted', 'تم رفض رسالة من مصدر غير موثق', {
            provider: sourceCheck.provider,
      ...
      // بما أن المصدر تم التحقق منه، نضع حالة التوثيق على verified
