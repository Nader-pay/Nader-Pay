...
import { Check, LogOut, Save, Server, Smartphone, X, Bell, Battery, Wifi, MessageSquare, Activity, Shield, ChevronLeft, Info, Download, RefreshCw } from 'lucide-react-native';
...
import { getAppVersion, checkLatestVersion } from '@/services/versionCheck';
  ...
  const handleCheckVersion = async () => {
    ...
    try {
      ...
      if (info?.updateRequired) {
        ...
        setError(`يتوفر إصدار جديد: v${info.latestVersion}`);
      } else {
        setLatestVersion(null);
        setMessage('التطبيق على أحدث إصدار');
      }
    } catch (err) {
    ...
    try {
      ...
      if (info?.downloadUrl) {
        ...
        if (canOpen) {
          ...
          setError('لا يمكن فتح رابط التحميل');
        }
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'فشل فتح رابط التحديث');
    }
  };
      ...
      <KeyboardAvoidingView behavior={process.env.EXPO_OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView className="flex-1 px-5" contentInsetAdjustmentBehavior="automatic" keyboardShouldPersistTaps="handled">
          ...
          <Section title="حول التطبيق">
            <View className="px-4 py-4 border border-border rounded-2xl bg-card mb-3 gap-3">
              <View className="flex-row items-center gap-3">
                <Info size={20} color="#6b7280" />
                <Text className="text-sm font-semibold text-foreground">Nader Pay Agent</Text>
              </View>
              <View className="flex-row justify-between">
                <Text className="text-sm text-muted-foreground">رقم الإصدار</Text>
                <Text className="text-sm font-medium text-foreground">v{appVersion || '—'}</Text>
              </View>
              {latestVersion ? (
                <View className="flex-row justify-between">
                  <Text className="text-sm text-muted-foreground">أحدث إصدار</Text>
                  <Text className="text-sm font-medium text-primary">v{latestVersion}</Text>
                </View>
              ) : null}
              <View className="flex-row gap-2 mt-1">
                ...
                >
                  ...
                  <Text className="text-sm font-medium text-foreground">التحقق من التحديثات</Text>
                </Pressable>
                {latestVersion ? (
                  <Pressable
                    onPress={handleUpdateNow}
                    className="flex-1 flex-row items-center justify-center gap-2 py-3 rounded-xl bg-primary active:opacity-80"
                  >
                    <Download size={16} color="#ffffff" />
                    <Text className="text-sm font-semibold text-primary-foreground">تحديث الآن</Text>
                  </Pressable>
                ) : null}
              </View>
            </View>
