...
import { Smartphone, RefreshCw, CheckCircle2, XCircle, MessageSquare, ShieldCheck, ShieldAlert, ChevronLeft } from 'lucide-react-native';
...
function StatusIcon({ status }: { status: ProviderSourceStatusType['status'] | undefined }) {
  if (status === 'verified') return <ShieldCheck size={18} color="#22c55e" />;
  if (status === 'failed') return <ShieldAlert size={18} color="#ef4444" />;
  return <ShieldAlert size={18} color="#6b7280" />;
}

export default function PaymentSourcesScreen() {
  const { state, refreshOrders, runDiagnostics } = useAgent();
  ...
  const [statuses, setStatuses] = useState<Record<ProviderName, ProviderSourceStatusType | null>>({
    vodafone_cash: null,
    orange_cash: null,
    insta_pay: null,
    bank_transfer: null,
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [rows, total, ...statusList] = await Promise.all([
      getIndexedSmsStats(),
      getIndexedSmsCount(),
      ...(['vodafone_cash', 'orange_cash', 'insta_pay', 'bank_transfer'] as ProviderName[]).map((p) =>
        getProviderSourceStatus(p)
      ),
    ]);
    ...
    setStats(next);
    const nextStatuses: Record<ProviderName, ProviderSourceStatusType | null> = {
      vodafone_cash: null,
      orange_cash: null,
      insta_pay: null,
      bank_transfer: null,
    ...
    for (const s of statusList) nextStatuses[s.providerId] = s;
          ...
          <View className="gap-4 pb-8">
            {PROVIDERS.map((p) => {
              ...
              return (
                <Pressable
                  key={p.key}
                  onPress={() => router.push(`/(app)/payment-source/${p.key}` as any)}
                  ...
                  <View className="px-4 py-5 border border-border rounded-2xl bg-card gap-3">
                    <View className="flex-row items-center justify-between">
                      ...
                      <View className="flex-row items-center gap-1">
                        ...
                        <Text className={`text-xs font-medium ${statusColor(status?.status)}`}>
                    ...
                    <Text className="text-xs text-muted-foreground">
                      آخر رسالة: {status?.lastMessageAt ? formatDate(status.lastMessageAt) : (s.lastAt ? formatDate(s.lastAt) : '—')}
                    </Text>
                  </View>
                </Pressable>
              );
