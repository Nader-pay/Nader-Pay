...
export async function getIndexedSmsMessagesByProvider(
  provider: ProviderName,
  limit = 200
): Promise<SmsMessage[]> {
  const db = await dbReady;
  const rows = (await db.getAllAsync(
    `SELECT id, body, originating_address, received_at, read_state
     FROM local_sms_index
     WHERE provider = ?
     ORDER BY received_at DESC
     LIMIT ?`,
  ...
  return rows.map((r) => ({
    id: r.id,
    body: r.body,
    originatingAddress: r.originating_address,
    date: r.received_at,
    readState: r.read_state ?? 1,
    threadId: 0,
    protocol: '',
  }));
}

export async function getIndexedSmsMessages(limit = 200): Promise<SmsMessage[]> {
  const db = await dbReady;
  const rows = (await db.getAllAsync(
    `SELECT id, body, originating_address, received_at, read_state
     FROM local_sms_index
